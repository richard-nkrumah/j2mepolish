...
public class TrieTest extends TestCase {
	...
	public void testEqualNodes() {
		Trie.Node root = new Trie.Node((char)0);
//		root.firstChild = new Trie.Node((char)0);
//		root.nextSibling = new Trie.Node((char)0);
		Node previous = root;
		for (int i = 0; i < 10; i++) {
			Node node = new Trie.Node((char)0);
			previous.firstChild = node;
			previous = node;
		}
	}
	
	private void visitNodes(Node node, boolean visitChild, boolean visitSibling) {
		//
	}
	
	private void populateNode(Node node, boolean addChild, boolean addSibling) {
		if(addChild) {
			node.firstChild = new Trie.Node((char)0);
		}
		if(addSibling) {
			node.nextSibling = new Trie.Node((char)0);
		}
	}
